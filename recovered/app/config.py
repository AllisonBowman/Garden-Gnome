...
class Settings(BaseSettings):
    ...
    # --- Amazon Associates (optional; product recommendations) ---
    # Left EMPTY on purpose. Product links still generate and still work
    # without it — they simply carry no tracking tag and earn nothing.
    # Shipping a plausible placeholder would credit whichever stranger owns
    # that tag for real customer purchases, so an unset tag yields an untagged
    # link, never a guessed one. GET /products/status reports the live state.
    amazon_associate_tag: str = ""
    amazon_marketplace: str = "amazon.com"
