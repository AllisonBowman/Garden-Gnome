...
14. A scientific name is the bare binomial. RHS renders author abbreviations in its H1 (e.g. "Adiantum pedatum L."); the abbreviation is authorship, not part of the name, so never copy it into scientific_name_accepted.
...
Second, sanity-check common_name: is it the name an admissible source's DEDICATED page for this exact species actually leads with (H1/title, or the first item of its common-name list), not a name the page assigns to a sibling species, not one bundled inside a multi-species umbrella term, and not a later entry in an "other common names" list? If not, refute it and say which name the dedicated source(s) actually lead with (common_name must never land null).
