...
const RECORD_SCHEMA = {
  ...
  properties: {
    ...
    is_edible: { type: ['boolean', 'null'] }, attracts_pollinators: { type: ['boolean', 'null'] },
    mature_height_in_min: { type: ['number', 'null'] }, mature_height_in_max: { type: ['number', 'null'] },
    mature_spread_in_min: { type: ['number', 'null'] }, mature_spread_in_max: { type: ['number', 'null'] },
...
17. MATURE SIZE, in INCHES, as a range. NC State's structured "Dimensions:" block ("Height: 8 ft. 0 in. - 12 ft. 0 in." / "Width: ...") is the anchor where the species has its own NC State page. Where it does not -- and a GENUS page is NOT a substitute, its range spans every species in the genus and is worthless for one of them -- RHS is the source, and RHS labels the fields "Max Height" and "Max Spread" in METRES, not "Ultimate height/spread": "Max Height         0.5-1 metres" is 19.7-39.4 inches (x 39.37). Convert and say so in the claim. An open-ended RHS value ("wider than 8 metres", "Higher than 12 metres") gives you no usable bound -- leave that end null. MoBot states size in Noteworthy Characteristics prose. Convert feet to inches yourself (3 ft = 36) and put the published low end in mature_height_in_min and the high end in mature_height_in_max, same for spread. A source that gives ONE figure ("to 6 ft") fills the max and leaves the min null -- never invent the other end, and never centre a single figure into a made-up range. RHS's "Ultimate height" is the size AT MATURITY over its stated time to reach it; a "5-10 years" qualifier does not make it a different field, but a size given explicitly for a named cultivar does, and cultivar sizes are not the species'. A trailing or climbing plant's "height" on a support is a trellis dimension, not the plant's own: if the page frames it that way, say so in unknowns and leave the field null. Label the citation with the STEM, not the bounds -- write "mature_height_in 36-72" and "mature_spread_in 24-36" -- so one citation supports both ends.
18. is_edible is TRUE only where a source states the plant, or a named part of it, is grown or used for food -- the vegetable, herb and fruit pages say so plainly. It is FALSE only where a source states it is not edible or is poisonous to eat; where no source addresses eating at all, leave it NULL. Null here is "no record", exactly as with toxic_to_pets, and a plant is never inferred edible from being in a genus of edibles. If edibility is conditional (cooked only, ripe fruit only, one part only while another is toxic), set true and put the condition in toxicity_detail and unknowns -- a bare true on a plant with a poisonous leaf is the dangerous half-truth this field must not tell.
19. attracts_pollinators is TRUE where a source's structured attracts/wildlife field or an unconditional statement names bees, butterflies, moths, hummingbirds or other pollinators. NC State's "Attracts" field and RHS's "attractive to pollinators" tag both count. "Wildlife value" prose about birds eating the SEED or deer browsing the foliage is not pollination -- do not read it as support. No mention at all is null, not false.
...
2b. STRUCTURAL CHECK -- SIZE AND FLAGS: mature_height_in_* and mature_spread_in_* are INCHES. Refute a value that is plainly the source's feet figure left unconverted (a 6 ft shrub recorded as 6), a metres figure left unconverted, a min above its max, a single published figure spread into an invented range, or a cultivar's size attributed to the species. REFUTE ANY SIZE TAKEN FROM A GENUS PAGE: NC State's genus entries carry a Dimensions block spanning every species in the genus (Hosta's reads 4 in - 2 ft 6 in, dwarf to giant), and a species-specific figure on any other permitted page beats it outright. Check the page title for a bare genus name before accepting its Dimensions. Refute is_edible or attracts_pollinators set false where the sources are simply silent -- silence is null. Refute attracts_pollinators supported only by seed-eating birds or browsing deer.
